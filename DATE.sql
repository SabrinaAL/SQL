...
-- Are all years evenly represented by the dataset?
 ...
 SELECT DATE_PART('year', occurred_at) ord_myear,  COUNT(total_amt_usd) total_number
 FROM orders
 GROUP BY 1
 ORDER BY 2 DESC;
...
-- Are all months evenly represented by the dataset?
 ...
 SELECT DATE_PART('month', occurred_at) ord_month,  COUNT(total_amt_usd) total_number
 FROM orders
 GROUP BY 1
 ORDER BY 2 DESC;
