...
SELECT COUNT(RIGHT(website, 4)) as count_website_account, RIGHT(website, 4) as website_account
FROM accounts 
GROUP BY 2

-- There is much debate about how much the name (or even the first letter of a company name) matters. Use the accounts table to pull the first letter of each company name to see the distribution of company names that begin with each letter (or number).
SELECT LEFT(name, 1) letter_name, COUNT(*) counts
FROM accounts 
GROUP BY 1
ORDER BY 2 DESC
...
SELECT SUM(t1.starts_vowels) vowels_ratio, SUM(t1.starts_consonants) everything_else

FROM (SELECT name, CASE WHEN LEFT(UPPER(name), 1) IN ('A','E','I','O', 'U') 
                          THEN 1 ELSE 0 END AS starts_vowels, 
            CASE WHEN LEFT(UPPER(name), 1) IN ('A','E','I','O', 'U') 
                          THEN 0 ELSE 1 END AS starts_consonants
     FROM accounts) t1                 
...
SELECT LEFT(primary_poc, (POSITION(' ' IN primary_poc))) first_name, RIGHT(primary_poc, (LENGTH(primary_poc) -POSITION(' ' IN primary_poc))) last_name
FROM accounts


-- FILLING empty columns with sf_crime_data
...
WHERE o.total IS NULL;
----------------------------------------------------------------
SELECT COALESCE(a.id, a.id) filled_id, a.name, a.website, a.lat, a.long, a.primary_poc, a.sales_rep_id, COALESCE(o.account_id, a.id) account_id, o.occurred_at, COALESCE(o.standard_qty, 0) standard_qty ,COALESCE(o.gloss_qty, 0) gloss_qty ,COALESCE(o.poster_qty, 0) poster_qty , o.total, o.standard_amt_usd, o.gloss_amt_usd, o.poster_amt_usd, o.total_amt_usd
FROM accounts a
LEFT JOIN orders o
ON a.id = o.account_id
WHERE o.total IS NULL;